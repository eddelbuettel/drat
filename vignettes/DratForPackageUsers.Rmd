...
we only need one piece of information: the name of the repository.  So
...
repos for Rcpp and my catch-all ghrr repo:
...
Sometimes we want to use repositories not confirming to the GitHub `drat`
pattern.  This use case may simply be a different URL.  Another common (and
very useful case) is for a locally hosted repository.

In this case we use the two argument form: the first argument sets the name
with the `repos` vector, and the second provides the local (filesystem-based)
or remote (web-based) URL:

```
drat::addRepo("workgroup", "file:///nfs/groups/groupABC/R/drat"RcppCore")
```

where the same local network repository example is used as in the example in
the [companion vignette](DratForPackageAuthors.html).

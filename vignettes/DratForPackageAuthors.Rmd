...
This vignette deals with the first case:  How to use [drat](http://dirk.eddelbuettel.com/code/drat.html) as package author.  A companion vignette for package users will be provided as well.
...
Lastly, if you have `git` (the command-line tool) or the wicked [git2r](http://cran.r-project.org/web/packages/git2r/index.html) package installed, then you can also use the `commit=TRUE` option to have the new files added and committed.  Neither of these pushes, so that last step is left to the user.

### GitHub and Travis Integration

[Colin Gillespie](http://www.mas.ncl.ac.uk/~ncsg3/) has provided a nice walk-through of how to have 
[Travis CI](https://travis-ci.org/) automagically push packages into a [drat](http://dirk.eddelbuettel.com/code/drat.html) repo. This is included as another (currently work-in-progress) vignette entitled
[Combining Drat And Travis](https://github.com/eddelbuettel/drat/blob/master/vignettes/CombiningDratAndTravis.Rmd) which can be found in the [drat](http://dirk.eddelbuettel.com/code/drat.html) package just like this vignette.

## Case 2: Using Another Server or Local Storage

Use of [drat](http://dirk.eddelbuettel.com/code/drat.html) is not limited to GitHub.  Any server you can 
...
This is similar to the usage described above. Suppose that you are part of groupABC which has access to 
directory on shared filesystem somewhere, say under `/nfs/groups/groupABC/` where you created a directory `drat` within a directory `R`.  We once again require that the resulting directory `/nfs/groups/groupABC/R/drat` contains a `src/contrib` directory structure.
...
drat::insertPackage("myPkg_0.5.tar.gz", "/nfs/groups/groupABC/R/drat")
```

This updates the PACKAGES file (and its compressed variant) after which the repository is ready to serve files.

## Summary

[drat](http://dirk.eddelbuettel.com/code/drat.html) permits package authors to add packages very easily to R package repositories. These repositories can be public, and GitHub provides a very natural option to serve a package repository via the web server based on the `gh-pages` branch of a drat. 

Repositories can also be local (and private) as well:  all that [drat](http://dirk.eddelbuettel.com/code/drat.html) requires to add packages is write access to a directory.  

Lastly, serving that directory as a repository then requires a web server (automatic via the GitHub repo option) or other file access.  How to access packages from [drat](http://dirk.eddelbuettel.com/code/drat.html) repository is described in a companion vignette.
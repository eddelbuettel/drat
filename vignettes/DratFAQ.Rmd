...
the main R repositpory network).
...
+ Because it is there and used by ever increasing numbers of authors.
+ Because it is pretty fast and reliable, no matter where you are.
...
known repository so that `update.packages` will see it.)  I think that is a
disservice to the users, and a repository can do better in a fundamental way
that access to (somewhat random) commits do.  Of course, one _could_ write
new code building on top of `install_github()` and adding the functionality.
But then, why? R _already_ has this functionality, and had it for decades:
